...
let Broadcaster = Class({
  ...
  get checked() broadcasterNS(this).options.checked,
  set checked(val) updateProperty(this, 'checked', !!val),
  get autocheck() broadcasterNS(this).options.autocheck,
  set autocheck(val) updateProperty(this, 'autocheck', !!val),
  get type() broadcasterNS(this).options.type,
  set type(val) updateProperty(this, 'type', val),
  get oncommand() broadcasterNS(this).options.oncommand,
  set oncommand(val) updateProperty(this, 'oncommand', val),
...
function updateBroadcasterAttributes(broadcaster, options) {
  ...
  if (options.type)
    broadcaster.setAttribute("type", options.type);
  ...
  if (options.oncommand)
    broadcaster.setAttribute("oncommand", options.oncommand);

  broadcaster.setAttribute('checked', !!options.checked);
  broadcaster.setAttribute('autocheck', !!options.autocheck);
...
function forEachBC(callback, broadcaster) {
  broadcasterNS(broadcaster).broadcasters.forEach(function(bc, i) {
    if (!bc) return;
    callback(bc, i, function(id) bc.ownerDocument.getElementById(id));
  });
}
